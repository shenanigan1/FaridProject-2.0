...
def test_candidate_only_lists_owned_evaluations(api_client):
    candidate = UserFactory.create(email="candidate-owned@example.com", password="Secret123")
    other_candidate = UserFactory.create(email="candidate-other@example.com", password="Secret123")
    manager = UserFactory.create(
        email="manager-owned-list@example.com",
        password="Secret123",
    ...
    owned_evaluation = Evaluation.objects.create(
        subject=candidate,
        assigned_to=manager,
    ...
    assert ids == {owned_evaluation.id}


def test_manager_only_lists_assigned_evaluations(api_client):
    manager = UserFactory.create(
        email="manager-assigned-list@example.com",
        password="Secret123",
        role=UserRoles.MANAGER,
    )
    other_manager = UserFactory.create(
        email="manager-other-list@example.com",
        password="Secret123",
        role=UserRoles.MANAGER,
    )
    subject = UserFactory.create(email="candidate-manager-list@example.com", password="Secret123")
    template_version = TemplateVersionFactory.create(
        template=TemplateFactory.create(), version=1
    )

    assigned_evaluation = Evaluation.objects.create(
        subject=subject,
        assigned_to=manager,
        template_version=template_version,
        status="in_progress",
    ...
    items = _unwrap_list_response(res.data)
    ids = {item["id"] for item in items}
    assert ids == {assigned_evaluation.id}


def test_candidate_cannot_retrieve_not_owned_evaluation(api_client):
    candidate = UserFactory.create(email="candidate-retrieve-owned@example.com", password="Secret123")
    other_candidate = UserFactory.create(email="candidate-retrieve-other@example.com", password="Secret123")
    manager = UserFactory.create(
        email="manager-retrieve-list@example.com",
        password="Secret123",
        role=UserRoles.MANAGER,
    )
    template_version = TemplateVersionFactory.create(
