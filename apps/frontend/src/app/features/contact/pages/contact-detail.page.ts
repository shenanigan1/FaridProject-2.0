    ...
    <section class="min-h-[calc(100vh-144px)] bg-slate-950 px-4 py-4 text-slate-100">
      <div class="mx-auto max-w-3xl space-y-4">
        <header class="flex items-center justify-between">
          <button routerLink="/contact" class="rounded-full border border-slate-700 px-3 py-1.5 text-sm">← Back</button>
          <button
            type="button"
            class="rounded-full border border-slate-700 px-3 py-1.5 text-sm"
        ...
        @if (pageMessage()) {
          <div class="rounded-xl border border-blue-500/25 bg-blue-500/10 p-3 text-sm text-blue-200">
            {{ pageMessage() }}
          </div>
        }
        ...
        @if (contact()) {
          <article class="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <div class="mt-4 grid grid-cols-3 gap-2">
              <button type="button" class="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold">Call</button>
              <button type="button" class="rounded-lg border border-slate-700 px-3 py-2 text-xs">Message</button>
              <button type="button" class="rounded-lg border border-slate-700 px-3 py-2 text-xs">Email</button>
            </div>

            <div class="mt-4 rounded-xl border border-slate-800 bg-slate-950/60 p-3 text-sm text-slate-300">
              TODO: add personal info block (phone, address, notes) when profile/contact endpoint fields are exposed.
            </div>
          </article>
        } @else {
            ...
            <div class="mt-4 space-y-3">
              ...
              @if (roleMenuOpen()) {
                <div class="rounded-xl border border-slate-700 bg-slate-950 p-3">
                  <label class="mb-1 block text-xs text-slate-400" for="role-select">New role</label>
                  <select id="role-select" class="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm" [value]="contact()!.role" (change)="onRoleChange($event)">
              ...
              <button type="button" class="w-full rounded-lg border border-slate-700 px-3 py-2 text-sm" (click)="dismissMenu()">Dismiss</button>
            </div>
          </div>
        </div>
      }
